 ...
 * 获取所有地级地市名称
 */
public class CityAdjacentParser {
    /** 整数 */
    ...
    //高德地图的API接口
    private static String mapUrl = "http://restapi.amap.com/v3/config/district?subdistrict=1&extensions=all&level=city&key=9d4f5c2078ba12cb9d9d09c4e81c95d0&s=rsv3&output=json&callback=jsonp_240122_&platform=JS&logversion=2.0&sdkversion=1.3&keywords=";

    //保存路径
    private static String saveUrl = "./cityname/";
...
//        saveCity("日喀则市");
    ...
    private static List<String> getAllCities() throws IOException{
        ...
        for (Region rp : province) {// 遍历省
            if(rp.getCode().equals("11") || rp.getCode().equals("12") ||
                    rp.getCode().equals("31") || rp.getCode().equals("50")){
                allCities.add(rp.getName());
            } else {
                //获取市名称
                List<Region> city = getCity(url + rp.getCode() + ".html");

                for (Region rc : city){
                    //如果是这两种情况还需要进步一步深入
                    if(rc.getName().equals("省直辖县级行政区划") ||
                            rc.getName().equals("自治区直辖县级行政区划")){
                        List<Region> county = getCounty(url + rp.getCode() + "/" + rc.getCode() + ".html");
                        for (Region cc: county){
                            allCities.add(cc.getName());
                        }
                    } else {
                        allCities.add(rc.getName());
                    }
                }
            }
        }
    ...
    private static List<Region> getProvince(String url) throws IOException {
        ...
        for (Element e : proviceTr) {
            ...
            for (Element element : tds) {
                Region region = new CityAdjacentParser().new Region();
                // region.setCode("13");
